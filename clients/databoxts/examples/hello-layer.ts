 ...
 * databoxts the way the SQL and S3 gateways are built on pkg/client.
 *
 ...
 *
 *     bun examples/hello-layer.ts --endpoint localhost:18443 --password devpass123
 *
 *     curl localhost:8091/world
 *     curl -X PUT localhost:8091/world -d 'Howdy'
 *     curl localhost:8091/world
 */

import { Databox } from "../src/index.ts";
...
const lease = await db.lease("hello-layer/instance-ts", {
  ttlMs: 10_000,
  onLost: (err) => console.error("lease lost:", err),
});
console.log(`holding lease hello-layer/instance-ts, fencing token ${lease.fencing}`);
...
const port = Number(arg("listen", "8091"));
Bun.serve({
  port,
  async fetch(req) {
